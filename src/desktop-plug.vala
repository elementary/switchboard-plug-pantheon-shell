...
public class GalaPlug : Pantheon.Switchboard.Plug
{
	...
	public GalaPlug ()
	{
		...
		mini_dur.value_changed.connect (() => AnimationSettings.get_default ().minimize_duration = (int)mini_dur.get_value () );
		...
		enable_anim.notify["active"].connect (() => {
			...
			mini_dur.sensitive = enable_anim.active;
		...
		anim_grid.attach (new LLabel.right (_("Minimize Duration:")), 0, 4, 1, 1);
		anim_grid.attach (mini_dur, 1, 4, 1, 1);
		anim_grid.attach (new LLabel.right (_("Workspace Switch Duration:")), 0, 5, 1, 1);
		anim_grid.attach (work_dur, 1, 5, 1, 1);
