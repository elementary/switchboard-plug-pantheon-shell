    ...
    */
    private void update_accountsservice () {
        try {
            ...
            if (!path.has_prefix (SYSTEM_BACKGROUNDS_PATH) && !path.has_prefix (get_local_bg_location ()) && !path.has_prefix (get_local_beautified_location ())) {
    ...
    private void update_checked_wallpaper (Gtk.FlowBox box, Gtk.FlowBoxChild child) {
        ...
        if (!(children is SolidColorContainer)) {
            var file = File.new_for_uri (children.uri);
            string filename = children.uri;

            if (settings.get_string ("picture-options") == "centered") {
				var wallpaper_maker = new Wallpaperize ();
				string output_name = "file://" + get_local_beautified_location () + file.get_basename ();
				stderr.printf ("%s : %s\n", filename, output_name);
			    wallpaper_maker.make_image (filename, output_name, 1920, 1080);

			    filename = output_name;
			}

			current_wallpaper_path = filename;
    ...
    private string get_local_beautified_location () {
        return Path.build_filename (Environment.get_user_data_dir (), "backgrounds-beautified") + "/";
    }
...
public class Wallpaperize : Object {
    ...
    public string make_image (string input, string output, int monitor_width, int monitor_height) {
        var surface = new Granite.Drawing.BufferSurface (W, H);
        try {
            var file = File.new_for_uri (input);
            image = new Gdk.Pixbuf.from_file (file.get_path ());
        } catch (Error e) {
            stderr.printf ("Error on image input: %s", e.message);
            return;
        }
        ...
        var file = File.new_for_uri (output);
        surface.surface.write_to_png (file.get_path ());
        return output;
    }
}
